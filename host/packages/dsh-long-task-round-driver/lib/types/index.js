...
export { renderLongTaskRoundPrompt } from "./prompt.js";
export const name = 'long-task-round-driver';
export const inject = ['agents', 'longTasks', 'sessions'];
...
function sameRound(source, round) {
    return source.taskId === round.taskId
...
export function apply(ctx) {
    ...
    function disarm(state) {
        try {
            ...
            if (task?.activation === 'armed')
                ctx.longTasks.disarm(state.agent);
        }
        catch (error) {
    ...
    function restoreOtherClaimed(agent, messages, messageId) {
        const retained = messages.filter(message => message.id !== messageId
            && !(message.source.kind === 'longTask' && message.source.round === 0));
    ...
    async function drive(state) {
        ...
        const task = currentTask(state);
        if (task === undefined || task.phase !== 'active' || task.activation !== 'armed')
            return;
        ...
        const message = createUserMessage({
            content,
            source: { kind: 'longTask', taskId: task.id, revision: task.revision, round },
        });
        const reservation = {
            taskId: task.id,
        ...
        try {
            ...
            if (latest !== undefined && latest.id === task.id && latest.revision === task.revision
                && latest.phase === 'active' && latest.activation === 'armed') {
                ctx.longTasks.block(agent, longTaskRef(latest), {
                    code: 'queue-failed',
    ...
    ctx.effect(function* () {
        ...
        ctx.on('agent/status', ({ agent, status }) => {
            ...
            if (status === 'idle') {
                ...
                if ((attempt?.phase === 'queued' || attempt?.phase === 'claimed' || attempt?.cancelled)
                    && task?.phase === 'active' && task.activation === 'armed') {
                    state.attempt = undefined;
                    try {
        ...
        function validReservation(state, content, source) {
            ...
            return ctx.fiber.state === 2 /* FiberState.ACTIVE */
                ...
                && task !== undefined && task.id === source.taskId && task.revision === source.revision
                && task.phase === 'active' && task.activation === 'armed'
                && source.round === task.roundsStarted + 1;
        }
        ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
            ...
            if (decision.kind === 'reject') {
                state.attempt = undefined;
                const task = currentTask(state);
                if (task !== undefined && task.id === source.taskId && task.revision === source.revision
                    && task.phase === 'active' && task.activation === 'armed') {
                    ctx.longTasks.block(agent, longTaskRef(task), {
