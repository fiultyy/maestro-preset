...
├── bin/                  # CLI 脚本(会话/编排操作面;dev-sync 同步镜像到 ~/.dsh/maestro/bin 作兜底)
│   ├── session-send      #   跨会话消息直发(DSHMSG 信封 → /api/session.prompt)
│   ├── session-spawn     #   起新会话并入 fleet
│   ├── session-purge     #   调 session-purge 插件的 purge 端口
│   ├── cb-send           #   对端回调投递(派发握手 ACK/DONE;HTTP 优先,文件桥兜底;HTTP 200≠送达,见 USAGE §3.3)
│   ├── fleet-probe       #   Orca 终端准入探测(termid 回报验证后才入编排列表)
│   └── dev-sync.sh       #   开发同步: 正向(装点+bin镜像+shared) / --reverse 逐文件回流 / --verify 三段报告
...
| `~/.dsh/maestro/bin/` | bin/ 镜像(cb-send 兜底安装点,dev-sync 维护;0006)+ 换代交接简报 `handoff-*.md` |
