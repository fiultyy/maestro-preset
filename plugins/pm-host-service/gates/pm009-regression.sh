#!/bin/bash
# pm009-regression.sh — PM-001..008 live gates re-run + PM-009 live health
# meta (G1 all-live + G3 bootstrap). [HF-010: this header previously still
# said "pm008-regression.sh / 0.8.0 / PM-008 baseline" and the SSE temp fell
# to /tmp/pm008-gates — both cleaned.]
...
# $PM_HOST_SERVICE_GATES_DIR/pm009/<label>/ (default
# ~/.dsh/maestro/logs/pm-host-service/gates) — never /tmp.
# Usage: pm009-regression.sh [label]
set -u
PASS=0; FAIL=0
...
echo "=== 0. deploy $VER via PM-002 restart gate (pid 更新 + 端口恒定, PMW2-G 钉港) ==="
...
curl -sN -m 12 "http://127.0.0.1:$NEW/subscribe?consumer=pm009-regression&kinds=act" >"$SSE" 2>/dev/null &
CURLPID=$!
sleep 1
...
echo "=== PM-009 live health meta (G1 all-live + G3 bootstrap) ==="
HJ=$(apiget $NEW /health)
echo "$HJ" | grep -q '"status":"ok"'; ok $? "PM-009 G1 live status ok"
for SRC in ledger tickets_md fleet dsh_api sessions flows; do
  echo "$HJ" | node -p "const j=JSON.parse(require('fs').readFileSync(0,'utf8'));j.sources.$SRC?.live" | grep -q true
  ok $? "PM-009 G1 live source $SRC live"
done
echo "$HJ" | node -p "const j=JSON.parse(require('fs').readFileSync(0,'utf8'));j.bootstrap.enabled" | grep -q '^enabled$'
ok $? "PM-009 G3 bootstrap enabled" "$(echo "$HJ" | node -p "const j=JSON.parse(require('fs').readFileSync(0,'utf8'));`enabled=${j.bootstrap.enabled} active=${j.bootstrap.active} unitFile=${j.bootstrap.unitFile.exists}`")"
echo "$HJ" | node -p "const j=JSON.parse(require('fs').readFileSync(0,'utf8'));j.bootstrap.active" | grep -q '^active$'
ok $? "PM-009 G3 bootstrap active"
echo "$HJ" | grep -q "\"version\":\"$VER\""; ok $? "PM-009 version in health" "want=$VER"
