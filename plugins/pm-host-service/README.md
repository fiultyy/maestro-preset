...
骨架路由：`GET /health` → 200 状态桩（完整 health/degraded 元端点属 PM-009）；非 GET → 405（只读红线）；其余 → 404（PM-003..006 端点占位）。stdout/stderr 走 journald；业务日志 `$MAESTRO_HOME/maestro/logs/pm-host-service/daemon.log`（>2MB 滚动）。
...
- 禁 npm 依赖、禁外发文件；op 投影端点与订阅属 PM-003..009
