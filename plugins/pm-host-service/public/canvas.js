...
// PMWEB-DAG 增量: 新增独立「图」tab (#view-dag) —— 票按 DAG 聚合的视图:
// 纯函数聚合在 cluster.js (双轨键 refs.run → deps 连通分量 → 前缀族/单票),
// 簇 = 可折叠超级节点 (簇头 = run_id|族名/票数/活跃数, 展开显成员票), 左侧簇
// list 联动染选 (选簇聚焦/选票高亮 deps 边+邻接, 其余淡出; selected 优先 hover)。
// 数据 GET /op/tickets (只读); SSE tickets 增量 refetch, 断 → 30s 轮询。
// 既有泳道画布 (#view-canvas, PMW2-2..4) 原样保留 —— 本文件只增不改其场景。
...
import { clusterTickets, clusterScene } from './cluster.js' // PMWEB-DAG 聚合层 (纯函数)
...
function renderDagCounts(degradedNote) {
  ...
  el.textContent = D2.scene
    ? `${D2.tickets.length} 票 → ${c.length} 簇 (run ${c.filter((x) => x.kind === 'run').length} / deps ${c.filter((x) => x.kind === 'deps').length} / 前缀 ${c.filter((x) => x.kind === 'prefix').length} / 单票 ${c.filter((x) => x.kind === 'single').length}) · 活跃 ${active} · 场景 ${D2.scene.nodes.length} 节点 / ${D2.scene.edges.length} 边 · 展开 ${D2.expanded.size}`
    : '加载中…'
...
async function rebuildDag(force) {
  D2.scene = clusterScene(D2.tickets, D2.expanded)
...
async function bootDag() {
  ...
  sec.innerHTML = `
    <div class="cv-head">
      ...
      <span class="dim small">聚合 DAG · ▸ 展开簇 / 双击簇折叠展开 · 点选簇聚焦、点选票染选 deps+邻接 · 空白点击取消 · 数据 GET /op/tickets (只读)</span>
    </div>
...
window.addEventListener('pm:sse', (e) => { // 复用 app.js SSE: 票面变化 → 图 tab refetch
  if (e.detail?.kind === 'tickets' || e.detail?.kind === 'act') dagRefetchDebounced()
})
window.addEventListener('pm:sse-state', (e) => {
...
async function boot() {
  ...
  bootDag() // PMWEB-DAG: 图 tab (聚合 DAG 场景) —— 与泳道画布并存, 互不影响
}
