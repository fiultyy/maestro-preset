...
- 开发模式: `bin/dev-sync.sh` 同步仓库→安装点 + bin/ 镜像(`~/.dsh/maestro/bin` 兜底)+ shared/ 技能面;`--reverse` 逐文件回流、`--verify` 三段漂移报告(见 USAGE §10)。**禁止软链接安装点**: discovery 的 `isDirectory()` 过滤会把 symlink 的 preset 从 roster 整个踢掉,新会话创建失败。
