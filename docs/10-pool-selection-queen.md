...
- **v6：queen 本体重投影 v2 + 人格轴独立**（2026-08-25）：①架构纠正——人格与预设正交：persona-axis host 插件（`~/.dsh/plugins/persona-axis/`，RPC `/persona-axis/rpc` list/current/select）在 agent 作用域以 systemPrompt.section 遮蔽 deployment persona，落账 `persona/selected` 事件（全文+版本）冷恢复重建，首条消息后锁定（prompt cache 保护）；池座席 v5 起纯消费该轴，export-to-preset/写默认/agentPresets.select 全部退役，预设轴零变化。②queen 本体 v1（wizard --role queen 通用采访员）缺投影机械，重投影 v2：基于六源（spawnAgentPrompt SOP+BEHAVIOR-SPACE 内核+CORE/ESSENCE+FLOW 复盘）手写成文——七类场景形状先验、十八项追问清单（agent_role+16 行为维+时间视野，自然语言化）、答案转译规则（强约束→硬性要求/硬底线→禁令/放松→反投射/涌现并入）、操作性合并律、冲突裁决序、派生者五铁律（职责链/人在环/派生专属/血缘如实/越界即停）+ 双铁律（产物零内部术语、灾难底线恒禁止）+ 消毒（双左花括号改写）；三门实跑 PASS（rt_projection_gates 真模块预检），incubate 同名归档 v1→v2，`vector19.agent_role=queen` 落 profile.json。池现状：`queen` v2（本体）+ `queen-v1` v1（Diff 审查员，derived-by=queen）+ 不加载
